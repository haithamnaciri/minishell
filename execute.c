...
/*   Updated: 2022/02/18 22:38:51 by hnaciri-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	s_exec_b_cmds(t_mini_data *mini_data,
	int **pipes, int index, int last_type)
{
	int		*fd_files;
	int		*her_pipe;
...
void	creat_childernes(t_data *data, int *i, int *l_type, int **pipes)
{
	while (data->num_childs < data->num_cmds)
	{
		...
		else if (data->pid[data->num_childs] == 0)
		{
			...
			{
				s_exec_b_cmds(&data->mini_cmds[data->num_childs],
					pipes, *i, *l_type);
				exit(g_data.errsv);
			}
			else
				executing(&data->mini_cmds[data->num_childs],
					pipes, *i, *l_type);
		}
		data->num_childs++;
	}
